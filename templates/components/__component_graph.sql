...
{
	'nodes': {{ (config.cur|query("select * from nodes_{0}".format(name))).data }},
	'edges': {{ (config.cur|query("select * from edges_{0}".format(name))).data }}
}
{% set res = config.cur|query_exec("drop view `nodes_{0}`".format(name)), config.cur|query_exec("drop view `edges_{0}`".format(name)) %}
