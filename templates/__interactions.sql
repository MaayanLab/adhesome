{% set substitutions %} {
	...
	"pmid": "<select onchange='window.open(this.options\\[this.selectedIndex\\].value,\"_blank\")'><option value=''>Source Article(s)</option>[% for i in pmid.split(' ') %]<option value='http://www.ncbi.nlm.nih.gov/entrez/query.fcgi?cmd=Retrieve&db=pubmed&dopt=Abstract&list_uids=[[ i ]]'>[[ i ]]</option>[% endfor %]</select>"
} {% endset %}
{% if name %}
